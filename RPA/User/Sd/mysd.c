...
void SD_DeInit(void);//SDIO所有引脚复位
...
void INTX_DISABLE(void);//关闭中断
void INTX_ENABLE(void);//开启中断
...
/*********************缓存区**************/
__align(4) u8 SDIO_DATA_BUFFER[BLOCK_SIZE];	
...
static void GPIO_Configuration(void)
{
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_SDIO, ENABLE);//72M
...
{		 
	...
	DMA_InitStructure.DMA_MemoryBaseAddr = (u32)(SDIO_DATA_BUFFER);  //DMA内存基地址
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralDST;  //数据传输方向，从内存读取发送到外设
	DMA_InitStructure.DMA_BufferSize = BLOCK_SIZE/4;  //DMA通道的DMA缓存的大小
...
{	  
   u32 status; 
	...
	while(1)
	{
		status=SDIO->STA;
		timeout--;
		if((status&((1<<0)|(1<<2)|(1<<6)))||timeout==0)break;//CRC错误/命令响应超时/已经收到响应(CRC校验成功)
	} 
	if(SDIO_GetFlagStatus(SDIO_FLAG_CTIMEOUT) != RESET||timeout==0)					//响应超时
	{																				    
